...
9) В Bitrix24 соблюдай структуру: эпик (базовая задача) -> подзадачи -> вложенные подзадачи; связи по Ганту обязательны.
10) После каждого push обязательно проверяй синхронизацию деплоя на VPS (`./scripts/vps/verify-sync.sh`).
11) При завершении задачи заполняй `Результат задачи` (что сделано + ссылка на commit).
12) Для эпиков включай автозакрытие основной задачи при закрытии подзадач; закрытый эпик помечай словом `Завершена` в названии.
```
