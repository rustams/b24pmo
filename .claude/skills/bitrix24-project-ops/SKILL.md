...
4. Нумерация иерархии обязательна:
- эпик: `Эпик N. ... [EPIC-xxx]`,
- задачи первого уровня в эпике N: `Задача N.1 ...`, `Задача N.2 ...`,
- вложенные задачи: `Задача N.1.1 ...`, `Задача N.1.2 ...` и т.д.
5. Эпик в Bitrix24 — это отдельная базовая задача верхнего уровня.
6. Задачи эпика оформляются как подзадачи эпика; вложенные уровни — через `parent`.
7. В описании эпика обязательно: состав задач, используемые skills/инструменты, критерий завершения эпика.
8. Описание задач — на русском языке.
9. При завершении задачи обязательно заполнить в Bitrix24 `Результат задачи`: что сделано + ссылка на commit.
10. Для эпиков включить опцию автозакрытия основной задачи при закрытии подзадач.
11. Если эпик закрыт, добавить в его название слово `Завершена`.
12. После `git push` обязательно запускать `./scripts/vps/verify-sync.sh`.
13. Перед стартом задачи обязательно проверить зависимости (`depends_on`) в roadmap:
- если зависимая задача не закрыта (`STATUS != 5`), ставить задачу в `blocked` и не начинать реализацию;
- после закрытия зависимости обновить контекст и только потом переводить задачу в работу.
14. Для cross-epic зависимостей действуют те же правила блокировки/разблокировки.
...
## Blocked Workflow (dependency gate)

- `blocked` состояние используется, если есть незавершенная зависимость.
- В `blocked` нельзя:
  - начинать кодовую реализацию,
  - менять статус на `В работе`.
- Для разблокировки требуется:
  - подтвержденное завершение зависимой задачи в репозитории и Bitrix24,
  - обновление контекста (`session-summary`, `decision-log`, `artifact-index`, epic handoff).
